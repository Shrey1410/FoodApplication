...
const LoginCustomer = () => {
  ...
  const handleonSubmit = async ()=>{
    ...
    try{
      const res = await axios.post(`${VITE_API_URL}/customer/login`, obj, {
    ...
    <div className="h-screen w-screen relative">
      ...
      <div className="absolute items-center flex flex-col justify-center w-full top-3/5 z-20 left-1/2 -translate-x-1/2 -translate-y-3/5">
        ...
        >
          ...
          <button
            className="btn bg-purple-500 hover:bg-purple-800 w-full text-white rounded-xl m-2"
            onClick={(e) => {
              e.preventDefault();
              handleonSubmit();
            }}
          ...
          <div className="text-center">
            <Link className="text-sm underline hover:text-gray-500" to="/registercustomer">
              Don't have account? Register Customer
            </Link>
          </div>
        </form>
      </div>
